...
parser.add_argument('--is_chain', '-is_chain', action='store_true', \
    default=False, \
    help='Pass this argument if there is chain information in the sample_details file.')
parser.add_argument('--size_gap', '-size_gap', metavar='size_gap', \
    default = 10000, \
    help='Max Distance greater than this will be eliminated')
parser.add_argument('--skip', '-skip', metavar='skip', \
    default=False, help='To get only amino acids count make this True')
...
def processFiles(filePath):
        ...
        for i in inFile:
            ...
            if(i[0:4].rstrip())=="ATOM" and (i[13:15].rstrip())=="CA" and (i[16]=='A'or i[16]==' ')and i[17:20]!= "UNK" :
                    aminoacidCount[counter] = i[17:20]
                        ...
                        if maxDist < int(args.size_gap):
                            ...
                            if args.needTriplets:  
                                line = (str(key_2) +"\t" + str(orig_aacd0) + "\t" + str(position0) + "\t" + \
                                    str(orig_aacd1) + "\t" + str(position1) + "\t" + \
                                    str(orig_aacd2) + "\t" + str(position2) + "\t" + str(classT1) + "\t" + str(Theta) + "\t" +\
        ...
        return (fileName,protLen,len(filesDict), sum(list(filesDict.values())))
...
if __name__ == '__main__':
    ...
    print("Working on theta: [{}], dist: [{}] \nSize Gap: [{}] \nUsing Chain Information?: [{}]"\
        .format(str(dTheta), str(dLen), str(args.size_gap), str(args.is_chain)))
